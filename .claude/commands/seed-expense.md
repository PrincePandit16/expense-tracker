...
---
Read database/db.py to understand the
expenses table
schema, the db connection pattern, and the
database
file name.
...
- user_id - integer
- count - integer, number of expenses to
create
- months - integer, how many past months to
spread them across

If any argument is missing or not a valid
integer, stop and say:
"Usage: /seed-expenses <user_id> <count><month>
...
1. Spreads expenses randbmly across the
past <months> months
2. Uses these categories with realistic
Indian descriptions
and amounts (₹):
- Food: 50-800
- Transport: 20-500
...
- Shopping: 200-5000
- Other: 50-1000
3. Distributes categories roughly
proportionally
...
6. Inserts all expenses in a single
transaction -
